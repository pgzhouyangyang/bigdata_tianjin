<template>
  <div class="charts-box">
    <div class="charts" id="myChartMember" v-show="clickNum == 0" style="height:100%"></div>
      ...
      <el-scrollbar class="page-component__scroll" style="height:100%">
        <table width="100%">
          ...
          <tbody>
            ...
            <tr v-if="memberData && !memberData.length">
              <td colspan="3">
...
<script>
import { getRynum } from "@/api/homeDetail";
...
export default {
  ...
  watch: {
    ...
    // 监听父组件数据
    memberData() {
      this.chartInit(this.memberData);
      if (this.clickNum > 0 && this.memberData.length) {
        let total = 0;
  ...
  methods: {
    ...
    chartInit(memberData = []) {
      // 基于准备好的dom，初始化echarts实例
      let myChart = this.$echarts.init(
        document.getElementById("myChartMember")
      );
      let data = [];
      let legend = [];
