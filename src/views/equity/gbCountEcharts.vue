        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-for="(item,index) in chartsData" :key="index">
              <td>{{item.orgname}}</td>
              <td>{{item["集体股东"] + item["个人股东"]}}</td>
              <td>
                <el-progress :percentage="computedPer(item['集体股东'] + item['个人股东'])"></el-progress>
              </td>
            </tr>
...
<script>
import { getGdSum } from "@/api/homeDetail";
